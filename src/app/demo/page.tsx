...
const sourceRows = [
  { name: "MeriPehchaan", detail: "Identity, address, mobile", provider: "meripehchaan" },
  { name: "DigiLocker", detail: "Class X & XII certificates", provider: "digilocker" },
  { name: "APAAR", detail: "Academic record · updated today", provider: "apaar" },
] satisfies Array<{ name: string; detail: string; provider: Exclude<SourceProvider, "profile"> }>;
...
function DemoTopbar({ backendReady, onNotifications }: { backendReady: boolean; onNotifications: () => void }) {
  return <header className="demo-topbar">
    ...
    <div className="demo-topbar-right"><span className="demo-badge"><Sparkles /> Synthetic demo{backendReady ? " · live data" : ""}</span><button className="icon-button" type="button" aria-label="Notifications" onClick={onNotifications}><Bell /></button><span className="avatar">AM</span></div>
  </header>;
}
...
function DemoSidebar({ step, onNavigate }: { step: DemoStep; onNavigate: (step: DemoStep) => void }) {
  return <aside className="demo-sidebar">
    ...
    <nav className="sidebar-nav" aria-label="Demo navigation">
      <button className={`sidebar-link ${step === "dashboard" ? "active" : ""}`} type="button" onClick={() => onNavigate("dashboard")}><LayoutDashboard /> Overview</button>
      <button className={`sidebar-link ${step === "form" ? "active" : ""}`} type="button" onClick={() => onNavigate("form")}><WalletCards /> My profile <Chip color="success" variant="soft" size="sm">98%</Chip></button>
      <button className={`sidebar-link ${step === "review" ? "active" : ""}`} type="button" onClick={() => onNavigate("review")}><FileText /> Applications <span className="sidebar-count">1</span></button>
      <button className={`sidebar-link ${step === "receipt" ? "active" : ""}`} type="button" onClick={() => onNavigate("receipt")}><ShieldCheck /> Consent log</button>
    </nav>
...
function JourneyHeader({ step, onBack }: { step: DemoStep; onBack: () => void }) {
  ...
  return <div className="journey-header"><div><button className="text-link" type="button" onClick={onBack}><ArrowLeft /> Back to overview</button><h1>{title}</h1><p>{sub}</p></div><Progress step={step} /></div>;
}
...
function FormStage({ issueResolved, onResolve, onSubmit, onBack }: { issueResolved: boolean; onResolve: () => void; onSubmit: () => void; onBack: () => void }) {
  ...
  return <><JourneyHeader step="form" onBack={onBack} /><div className="journey-grid"><section className="form-shell"><div className="form-top"><strong>National STEM Entrance 2026</strong><span><BadgeCheck /> 35 fields prefilled</span></div><div className="form-content"><div className="eyebrow"><span className="eyebrow-dot" /> Review before submission</div><h2>Everything in place.</h2><p>We matched your profile to this synthetic application. You only need to confirm what is new.</p>{!issueResolved ? <div className="form-alert"><CircleAlert /><div><strong>One field needs your attention</strong>Your income certificate was issued 11 months ago. <button type="button" onClick={onResolve}>Confirm it is still valid</button> to continue.</div></div> : <div className="form-alert resolved"><CheckCircle2 /><div><strong>Field confirmed</strong>Your profile is complete and ready for final review.</div></div>}<div className="prefill-grid">{applicationFields.map(([label, value]) => <div className="fake-field verified" key={label}><label>{label}</label><span>{value}</span><CheckCircle2 className="verified-mark" /></div>)}<div className="fake-field"><label>Exam city preference · You choose</label><span>Pune</span></div><div className="fake-field"><label>Declaration · You confirm</label><span>I agree to the terms</span></div></div><div className="form-actions"><small><LockKeyhole /> Final consent happens in the next step</small><Button className="primary-action" variant="primary" isDisabled={!issueResolved} onPress={() => setConfirmationOpen(true)}>Review sharing scope <Send className="inline-arrow" /></Button></div></div></section><aside className="journey-side-note"><div className="panel-kicker">What just happened</div><h2>A messy form became a clear packet.</h2><p>Instead of typing the same identity, family, and academic details again, you reviewed the source and confirmed the two things that only you can decide.</p><div className="side-stat-list"><div className="side-stat"><span>Fields mapped</span><strong>35 / 38</strong></div><div className="side-stat"><span>Sources checked</span><strong>3</strong></div><div className="side-stat"><span>Extra typing</span><strong>3 fields</strong></div><div className="side-stat"><span>Consent scope</span><strong>Exam only</strong></div></div></aside></div>{confirmationOpen ? <div className="ao-modal-backdrop"><section className="ao-modal ao-affirmation-dialog" role="dialog" aria-modal="true" aria-labelledby="demo-affirmation-title"><div className="ao-dialog-head"><div><div className="ao-card-kicker">Final affirmation · synthetic demo</div><h2 id="demo-affirmation-title">Share 38 fields and 2 documents?</h2></div><button className="ao-topbar-icon" type="button" onClick={() => setConfirmationOpen(false)} aria-label="Close affirmation"><X /></button></div><p className="ao-modal-lede">This records a synthetic ApplyOnce submission and receipt. It does not contact or claim confirmation from a real external portal.</p><div className="ao-affirmation-scope"><div><span>Recipient</span><strong>National STEM Entrance 2026 · synthetic</strong></div><div><span>Purpose</span><strong>Entrance application demonstration</strong></div><div><span>Shared now</span><strong>38 fields · 2 documents</strong></div><div><span>Receipt</span><strong>ApplyOnce demo receipt only</strong></div></div><div className="ao-modal-actions"><button className="ao-button ao-button--quiet" type="button" onClick={() => setConfirmationOpen(false)}>Go back and review</button><button className="ao-button ao-button--primary" type="button" onClick={onSubmit}>Confirm, share and submit<ShieldCheck /></button></div></section></div> : null}</>;
}
...
function ReceiptStage({ onRestart, onBack }: { onRestart: () => void; onBack: () => void }) {
  ...
  const copyReceipt = async () => {
    try {
      ...
      await navigator.clipboard.writeText("AO-DEMO-004281");
      setCopied(true);
  ...
  const saveReceipt = () => {
    triggerDownload(new Blob([JSON.stringify({ product: "ApplyOnce", application: "National STEM Entrance 2026", receiptCode: "AO-DEMO-004281", submittedAt: "2026-08-29T20:18:00+05:30", status: "applyonce_demo_recorded", externalReceiptConfirmed: false, consentScope: "Exam only" }, null, 2)], { type: "application/json" }), "ao-demo-004281-receipt.json");
    setSaved(true);
  };

  return <><JourneyHeader step="receipt" onBack={onBack} /><div className="receipt-wrap"><section className="receipt-card"><div className="receipt-hero"><span className="success-orb"><Check /></span><h1>ApplyOnce submission recorded.</h1><p>Your synthetic packet and consent snapshot are saved. This demo receipt does not claim confirmation from a real external portal.</p></div><div className="receipt-details"><div className="receipt-detail"><span>Recorded at</span><strong>29 Aug 2026 · 8:18 PM</strong></div><div className="receipt-detail"><span>ApplyOnce receipt</span><strong>AO-DEMO-004281</strong></div><div className="receipt-detail"><span>External confirmation</span><strong>Not claimed</strong></div></div><div className="receipt-footer"><span className="receipt-id">RECEIPT / AO-DEMO-004281</span><div className="receipt-actions"><button className="secondary-action" type="button" onClick={saveReceipt}><Download className="inline-arrow" /> {saved ? "Receipt saved" : "Save receipt"}</button><button className="secondary-action" type="button" onClick={() => void copyReceipt()}><Copy className="inline-arrow" /> {copied ? "Copied" : "Copy ID"}</button></div></div></section><aside className="next-card"><div className="panel-kicker">Consent trail</div><h2>Clear after you apply.</h2><p>Here is the proof of what moved, why it moved, and what is still under your control.</p><div className="receipt-timeline"><div className="receipt-timeline-row"><CheckCircle2 /><span>3 synthetic sources mapped to your profile</span></div><div className="receipt-timeline-row"><CheckCircle2 /><span>38 fields approved for one stated purpose</span></div><div className="receipt-timeline-row"><CheckCircle2 /><span>ApplyOnce demo receipt generated</span></div><div className="receipt-timeline-row"><LockKeyhole /><span>External portal confirmation remains separate</span></div></div><Button className="primary-action" variant="primary" onPress={onRestart} style={{ marginTop: 25, width: "100%" }}>Run it again <ArrowRight className="inline-arrow" /></Button></aside></div></>;
}

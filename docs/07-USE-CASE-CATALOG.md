...
- **Integration mode** — `SDK` (partner runs `@applyonce/sdk`'s "Apply with ApplyOnce" button, live consent+payload flow, `docs/05-API-AND-FLOWS.md` F2) · `Extension` (Chrome MV3 recipe autofills a legacy portal with no API, F3) · `Hosted form` (ApplyOnce Forms — a partner with no dev team picks fields, ApplyOnce hosts the form itself, P3) · `Manual tracker` (no integration exists; citizen applies elsewhere, ApplyOnce only tracks the `applications` row, deadline, and documents).
...
| Jeevan ApplyOnce (digital life certificate, annual) | Delegate (elder) or self | `identity.aadhaar_last4`, `identity.dob`, `bank.primary.*` | Extension | ✅ |
...
- **Rural / low-bandwidth.** Every SDK/Extension flow needs a Manual-tracker-equivalent fallback: a ApplyOnce-generated, pre-filled PDF or a short share-code a CSC/VLE operator can use offline, so the CSC channel becomes a paid distribution partner (research 04 §C) rather than a bypass of ApplyOnce's consent guarantees.

...
| Aadhaar Act constraints | Online auth requires an approved AUA/KUA/Sub-AUA path; real offline e-KYC/App VC verification requires OVSE registration | Keep real Aadhaar verification disabled until the appropriate UIDAI registration/approval is complete |
...
5. **"How do you launch before regulated integrations?"** Launch ApplyOnce-hosted forms, partner APIs, manual non-Aadhaar evidence and user-controlled autofill first. Run synthetic provider demos while DigiLocker Requester and UIDAI OVSE approvals are pending. AA, online Aadhaar and ABDM remain separate regulated expansion tracks.

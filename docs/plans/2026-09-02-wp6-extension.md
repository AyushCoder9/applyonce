...
- `token/route.ts` — `POST`, `citizen()` session required, inserts a `session` row (`id/token = randomToken`, `userAgent:'applyonce-extension'`, `expiresAt +30d`), returns `{token, expiresAt, user, profiles}`.
- `fill-plan/route.ts` — `GET`, Bearer. `?recipe&profile&keys=a,b,c&stepUp=`. Decrypts via `getDek`+`getFacts`. sensitive keys need `stepUp === MOCK_OTP` (mock mode) else omitted → `missing`. `labels` via `@applyonce/ui` `fmtValue`.
...
- `apps/web/src/app/app/extension/connect/page.tsx` + client component — mint token, `postMessage` handshake, copy fallback, `APPLYONCE_EXT_CONNECTED` listener.
...
- `src/content/handshake.ts` — listens `APPLYONCE_EXT_TOKEN` on app origin, forwards to background, replies `APPLYONCE_EXT_CONNECTED`.
...
- `src/popup/*` — React popup: connected state, recipe status, fill button + step-up OTP prompt, attach-from-ApplyOnce list, disconnect.
...
## Verify
- `pnpm --filter @applyonce/extension test` (vitest, pure fns).
- `pnpm --filter @applyonce/extension typecheck` + `build` → `dist/manifest.json` valid.

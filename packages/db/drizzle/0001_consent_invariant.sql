...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'APPLYONCE_NO_CONSENT: share requires a consent row'; END IF;
  IF c.revoked_at IS NOT NULL THEN RAISE EXCEPTION 'APPLYONCE_CONSENT_REVOKED'; END IF;
  IF c.expires_at < now() THEN RAISE EXCEPTION 'APPLYONCE_CONSENT_EXPIRED'; END IF;
  IF NOT (NEW.shared_keys <@ c.scope) THEN RAISE EXCEPTION 'APPLYONCE_SCOPE_EXCEEDED: shared keys not within consent scope'; END IF;
  RETURN NEW;
...
BEGIN RAISE EXCEPTION 'APPLYONCE_AUDIT_IMMUTABLE'; END $$ LANGUAGE plpgsql;
--> statement-breakpoint

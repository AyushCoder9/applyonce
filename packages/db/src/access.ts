...
export async function listAccessibleProfiles(userId: string) {
  const [own, claimed] = await Promise.all([
    db.select().from(t.profiles).where(and(eq(t.profiles.ownerUserId, userId), eq(t.profiles.status, "active"), eq(t.profiles.kind, "self"), isNull(t.profiles.claimedByUserId))),
    db.select().from(t.profiles).where(and(eq(t.profiles.claimedByUserId, userId), eq(t.profiles.status, "active"))),
  ]);

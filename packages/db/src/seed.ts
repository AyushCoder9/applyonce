...
const sysKey = () => Buffer.from(hkdfSync("sha256", kekFromEnv(), "praman", "system-payload-key", 32));
...
async function seedPerson(p: DemoPerson) {
  ...
  const [prof] = await db.insert(t.profiles).values({ ownerUserId: p.id, kind: "self", displayName: p.name, dobYear: Number(p.dob.slice(0, 4)) }).returning();
...
async function seedDependent(ownerUserId: string, guardianPid: string, d: { name: string; dob: string; facts: Record<string, unknown> }, basis: "minor" | "elder_consent", relation: string, scope: string[]) {
  const [prof] = await db.insert(t.profiles).values({ ownerUserId, kind: "dependent", displayName: d.name, dobYear: Number(d.dob.slice(0, 4)) }).returning();
  await db.insert(t.relations).values({ guardianProfileId: guardianPid, wardProfileId: prof!.id, relation, basis, scope, validUntil: basis === "elder_consent" ? days(365) : null });
...
async function seedPartner(o: { slug: string; name: string; kind: typeof t.partnerKind.enumValues[number]; website: string; apiKey: string; webhookSecret?: string; webhookUrl?: string; ownerUserId: string }) {
  const [p] = await db.insert(t.partners).values({ slug: o.slug, name: o.name, legalName: o.name, kind: o.kind, website: o.website, status: "verified", dpoEmail: `dpo@${o.slug}.example` }).returning();
  await db.insert(t.partnerMembers).values({ partnerId: p!.id, userId: o.ownerUserId, role: "owner" });
  await db.insert(t.partnerApiKeys).values({ partnerId: p!.id, env: "sandbox", keyHash: sha256(o.apiKey), prefix: o.apiKey.slice(0, 14), label: "Demo sandbox key" });
  if (o.webhookUrl && o.webhookSecret) await db.insert(t.partnerWebhooks).values({ partnerId: p!.id, url: o.webhookUrl, secretEnc: encrypt(sysKey(), o.webhookSecret, `webhook:${p!.id}`) });
  return p!.id;
}
...
async function main() {
  ...
  await db.insert(t.user).values({ id: "usr_admin", name: "Praman Ops", email: "ops@praman.example", emailVerified: true, phoneNumber: "+919000000000", phoneNumberVerified: true, role: "admin" }).onConflictDoNothing();
  await db.insert(t.profiles).values({ ownerUserId: "usr_bta_admin", kind: "self", displayName: "BTA Admin" });
  await db.insert(t.profiles).values({ ownerUserId: "usr_admin", kind: "self", displayName: "Praman Ops" });

  const demoUrl = process.env.NEXT_PUBLIC_DEMO_PORTAL_URL ?? "http://localhost:3101";
  const bta = await seedPartner({ slug: "bta", name: "Bharat Test Agency", kind: "exam_board", website: "https://bta.example", apiKey: process.env.BTA_PRAMAN_API_KEY ?? "pk_sandbox_bta_demo_key_0001", webhookSecret: process.env.BTA_WEBHOOK_SECRET ?? "whsec_bta_demo_0001", webhookUrl: `${demoUrl}/api/praman/webhook`, ownerUserId: "usr_bta_admin" });
  ...
  const [btaForm] = await db.insert(t.forms).values({ partnerId: bta, slug: process.env.BTA_PRAMAN_FORM_ID ?? "bta-jee-2026", name: "BTA-JEE 2026 Registration", description: "Joint Entrance Examination (Sample) — Session 1", purpose: "exam_application", kind: "exam",
    ...
    retentionDays: 730, redirectUrl: `${demoUrl}/apply/return`, webhookUrl: `${demoUrl}/api/praman/webhook`, deadlineAt: days(21) }).returning();

  await db.insert(t.forms).values({ partnerId: nova, slug: "nova-btech-2026", name: "Nova University B.Tech Admissions 2026", purpose: "college_admission", kind: "admission",
  ...
  await db.insert(t.forms).values({ partnerId: bank, slug: "savings-account-kyc", name: "Savings Account KYC", purpose: "kyc_financial", kind: "kyc",
  ...
  const [app] = await db.insert(t.applications).values({ profileId: aarav.pid, partnerId: nova, title: "Nova University B.Tech Admissions 2026", orgName: "Nova University", kind: "admission", status: "under_review", externalRef: "NOVA-2026-001742", submittedAt: days(-6), deadlineAt: days(45), source: "sdk" }).returning();
  await db.insert(t.applicationEvents).values([
    { applicationId: app!.id, type: "created", title: "Application submitted via Praman", actor: "citizen", createdAt: days(-6) },
  ...
  await db.insert(t.flags).values([{ key: "extension", enabled: true }, { key: "partner_self_serve", enabled: true }, { key: "hindi_ui", enabled: true }]);

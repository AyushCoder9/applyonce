...
import { EmptyState, type Locale } from "@praman/ui";
...
const rel = (d: string, l: Locale) => { const m = Math.round((Date.now() - new Date(d).getTime()) / 60000); if (m < 60) return l === "hi" ? `${m} मिनट पहले` : `${m}m ago`; const h = Math.round(m / 60); if (h < 24) return l === "hi" ? `${h} घंटे पहले` : `${h}h ago`; return new Date(d).toLocaleDateString(l === "hi" ? "hi-IN" : "en-IN", { day: "numeric", month: "short" }); };

export function NotificationsList({ items, locale: l }: { items: Notif[]; locale: Locale }) {
    ...
    <section className="mb-6"><h2 className="mb-2 text-xs font-semibold uppercase tracking-[0.04em] text-ink-3">{title}</h2>
      <ul className="card divide-y divide-line">{rows.map((n, i) => { const inner = (
        <div className="flex items-start gap-3 px-4 py-3">
          ...
          <span className="shrink-0 text-xs text-ink-3">{rel(n.createdAt, l)}</span>
        </div>); return (

...
import { SourceChip } from '@praman/ui';
...
export function ReadinessPanel({ report, formId, profileId, aiEnabled, locale = 'en' }: { report: Report; formId: string; profileId: string; aiEnabled: boolean; locale?: 'en'|'hi' }) {
  ...
  return <div className="grid gap-5" data-testid="readiness-panel">
    ...
    <div className="grid gap-5 lg:grid-cols-[3fr_2fr]">
      ...
      <aside className="card flex flex-col p-5 sm:p-6"><div className="flex items-center gap-2"><Sparkles className="size-5 text-brand-600"/><h3 className="font-display text-xl font-bold">Ask Praman</h3></div><p className="mt-2 text-sm text-ink-2">A clear explanation, grounded in this form’s evidence checks.</p><p className="mt-2 text-xs font-medium text-brand-700">{aiEnabled ? 'AI explanations available' : 'Local guide · works without an AI key'}</p>

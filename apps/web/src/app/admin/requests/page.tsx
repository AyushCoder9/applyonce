      ...
      <div className="card overflow-x-auto"><table className="w-full min-w-[820px] text-sm">
        ...
        <tbody>{rows.length === 0 ? <tr><td colSpan={6} className="px-4 py-6 text-center text-ink-2">Queue is empty.</td></tr> : rows.map(({ r, u }) => (
          <tr key={r.id} className="border-b border-line align-top last:border-0">
            ...
            <td className="px-4 py-3">{r.requestedAt.toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</td><td className={`px-4 py-3 tabular ${sla(r.requestedAt) < 7 && r.status === "pending" ? "font-bold text-danger-500" : ""}`}>{r.status === "done" ? "—" : `${sla(r.requestedAt)} d left`}</td>

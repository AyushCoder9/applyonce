  ...
  return (
    <div className="min-h-dvh flex flex-col">
      <header className="mx-auto flex w-full max-w-[1120px] items-center justify-between px-4 py-4 md:px-6">
        <Link href="/" className="flex items-center gap-2"><img src="/icon.svg" alt="" className="size-8 rounded-lg" /><span className="font-display text-xl font-bold">Praman</span></Link>
      ...
      <footer className="border-t border-line bg-surface">
        <div className="mx-auto grid w-full max-w-[1120px] gap-6 px-4 py-10 text-sm text-ink-2 md:grid-cols-4 md:px-6">
          <div><div className="font-display text-lg font-bold text-ink">Praman</div><p className="mt-2">Verify once. Apply anywhere.</p><p className="mt-1 hi">एक बार सत्यापित करें। कहीं भी आवेदन करें।</p></div>
          ...
          <div className="grid gap-1"><span className="font-semibold text-ink">Legal</span><Link href="/terms">Terms</Link><span>© {new Date().getFullYear()} Praman</span></div>
        </div>
      </footer>
    </div>
  );

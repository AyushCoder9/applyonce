    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <Container className="grid items-center gap-12 py-14 md:grid-cols-[1.05fr_1fr] md:py-24">
          <div className="rise">
            <div className="inline-flex items-center gap-2 rounded-pill border border-line bg-surface px-3 py-1 text-xs font-semibold text-ink-2"><span className="size-1.5 rounded-pill bg-verified-500" />Working sandbox · Evidence before submission</div>
      ...
      <Container className="pb-4">
        <dl className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {[["Citizens", fmt(s.citizens)], ["Verified facts", fmt(s.verified)], ["Sandbox institutions", fmt(s.partners)], ["Consented shares", fmt(s.shares)]].map(([l, v]) => <div key={l} className="card px-5 py-4"><dt className="text-sm text-ink-2">{l}</dt><dd className="mt-0.5 font-display text-3xl font-bold tabular">{v}</dd></div>)}
        </dl>
        <p className="mt-2 text-xs text-ink-3">Sandbox database counts · includes seeded records · refreshed every 5 minutes.</p>
      </Container>
      ...
      {/* Trust */}
      <Section className="bg-surface border-y border-line" eyebrow="Built around explicit consent" title="Your data. Your consent. Our receipts." blurb="Every share is itemised, signed, logged and revocable. The sandbox makes each step visible.">
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ...
            { I: ShieldCheck, t: "Source on every fact", d: "Source labels distinguish provider evidence from information you typed. In the sandbox, provider evidence is simulated and attachments are marked as samples." },

...
export default async function Status() {
  const [db, portal, worker] = await Promise.all([pingDb(), checkDemoPortal(), workerHealth()]);
  const allOk = db.ok && portal.ok && worker.ok;
  ...
  return (
    <Container className="py-14 md:py-20">
      ...
      <ul className="card mt-6 divide-y divide-line">
        {[["Web app", true, `${region} · ${commit}`], ["Database (Postgres)", db.ok, db.ok ? `${db.ms} ms` : db.error ?? "down"], ["BTA application state", portal.ok, portal.redis.ok ? `Redis-backed · ${portal.redis.ms ?? "—"} ms` : portal.configured ? "portal state unavailable" : "portal not configured"], ["Worker", worker.ok, worker.note]].map(([n, ok, note]) => <li key={String(n)} className="flex items-center justify-between gap-4 px-5 py-3"><span className="flex items-center gap-3"><Dot ok={!!ok} />{n}</span><span className="text-right text-sm text-ink-3">{note}</span></li>)}
      </ul>
      ...
      <p className="mt-8 text-xs text-ink-3">Checked at {new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST · build {commit} · Incidents: status@applyonce.in</p>
    </Container>
  );
}

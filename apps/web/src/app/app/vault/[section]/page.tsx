...
export async function generateMetadata({ params }: { params: Promise<{ section: string }> }) { const { section } = await params; return { title: (SECTIONS as readonly string[]).includes(section) ? sectionMeta(section as Section).label.en : "My profile" }; }
...
export default async function VaultSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  return (
    <div>
      <PageHeader back={{ href: "/app/vault", label: tr(locale, "Profile", "प्रोफ़ाइल") }} eyebrow={a.profile.displayName} title={meta.label[locale]} subtitle={meta.blurb[locale]} actions={<ProgressRing value={c.filled} max={c.total} label={tr(locale, `${c.verified} verified`, `${c.verified} सत्यापित`)} />} />

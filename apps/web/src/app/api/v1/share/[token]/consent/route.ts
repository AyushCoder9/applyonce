...
import { db, t, eq, inArray, audit, getDek, getFacts, putFact, systemDek } from "@praman/db";
import { encrypt, sha256, canonicalHash } from "@praman/crypto";
import { field, isFactKey, scopeForPurpose, type PramanPayload, type SharedFact } from "@praman/schema";
...
export const POST = handler(async (req, { params }) => {
  ...
  const { user, profile, ownerUserId } = await citizen(req, { profileId: b.profileId, stepUp: true });
  ...
  const accepted = [...new Set([...required, ...b.acceptedFields.filter((k) => allowed.includes(k))])];
  const customErrors = Object.fromEntries(form.customFields.filter((c) => c.required && (b.customAnswers[c.id] == null || b.customAnswers[c.id] === "" || b.customAnswers[c.id] === false)).map((c) => [c.id, "Required"]));
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const sharedKeys = [...new Set(facts.map((f) => f.key))];

    const docIds = [...new Set(facts.flatMap((f) => [f.evidenceDocumentId, field(f.key).type === "file_ref" ? String(f.value) : null]).filter((x): x is string => !!x))];
    const docs = docIds.length ? await tx.select({ id: t.documents.id, sha256: t.documents.sha256, title: t.documents.title }).from(t.documents).where(inArray(t.documents.id, docIds)) : [];
    ...
    const sharedFacts: SharedFact[] = facts.map((f) => ({ key: f.key, value: f.value, source: f.source, verifiedBy: f.verifiedBy ?? null, verifiedAt: f.verifiedAt ?? null, evidence: doc(field(f.key).type === "file_ref" ? String(f.value) : f.evidenceDocumentId) }));
  ...
  await flushDeliveries(result.deliveries);
  return ok({ return_url: withQuery(ss.returnUrl, { share_token: result.token, state: ss.state }), consent_id: result.consentId, application_id: result.applicationId, shared: result.shared });
});

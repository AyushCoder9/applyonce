...
export const POST = handler(async (req, { params }) => {
  ...
  if (doc.origin !== "upload") throw new ApiError(403, "ISSUER_DOCUMENT_READONLY");
  if (doc.status === "ready") return ok({ documentId: doc.id, status: "ready" });
  if (doc.status === "rejected") throw new ApiError(409, "DOCUMENT_REJECTED", "Upload a new file.");
  const b = z.object({ size: z.number().int().positive().max(15 * 1024 * 1024).optional(), sha256: z.string().regex(/^[a-f0-9]{64}$/).optional() }).parse(await req.json().catch(() => ({})));

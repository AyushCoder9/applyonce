...
export const GET = handler(async (req, { params }) => {
  ...
  const docs = (await db.select().from(t.documents).where(eq(t.documents.profileId, a.profile.id)).orderBy(desc(t.documents.createdAt))).filter(d => documentAllowed(a.scope, d.docType));

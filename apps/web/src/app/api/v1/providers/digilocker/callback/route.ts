...
import { openOAuthTransaction } from "@/lib/oauth-transaction";
...
export const GET = handler(async (req) => {
  ...
  const saved = c ? (() => { try { return openOAuthTransaction(c); } catch { return null; } })() : null;
  ...
  if (!saved || saved.transaction.state !== state || !code) return bounce("digilocker_state");
  const a = await citizen(req, { profileId: saved.profileId });
  if (a.user.id !== saved.userId) return bounce("digilocker_session");
  const done = await providers.digilocker.completeAuth(saved.transaction, code).catch(() => null);

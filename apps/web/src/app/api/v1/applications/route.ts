...
/** POST /api/v1/applications — track an application made outside ApplyOnce (manual or extension). */
export const POST = handler(async (req) => {
  ...
  await db.insert(t.applicationEvents).values({ applicationId: a!.id, type: "created", title: b.source === "extension" ? "Filled with the ApplyOnce extension" : "Added to tracker", body: b.note ?? (b.external_ref ? `Reference ${b.external_ref}` : null), actor: "citizen" });

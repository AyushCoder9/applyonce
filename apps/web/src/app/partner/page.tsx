...
import { db, t, eq, desc, gte, and, inArray } from "@praman/db";
import type { PramanPayload } from "@praman/schema";
import { decryptString } from "@praman/crypto";
import { systemDek } from "@praman/db";
import { PageHeader, StatTile, Callout, fmtDate } from "@praman/ui";
...
export default async function PartnerOverview() {
  ...
  for (const s of recentShares) { try { const p = decodeJws<PramanPayload>(decryptString(systemDek(), s.payloadEnc, `share:${s.id}`)); total += p.facts.length; verified += p.facts.filter((f) => f.source === "issuer_verified" || f.source === "provider_verified").length; } catch { /* skip */ } }
  ...
  return (
    <>
      ...
      {partner.status === "pending" && <Callout tone="warning" title="Your organisation is being verified">Sandbox keys work now. Live keys unlock after Praman checks your registration number. Usually 1–2 working days.</Callout>}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <section className="card p-5 text-sm">
          ...
          <ol className="grid list-decimal gap-2 pl-5 text-ink-2">
            ...
            <li>Drop the button on your portal: <code className="font-mono text-xs">&lt;button data-praman-form="slug"&gt;</code></li>
            <li>Exchange the share token server-side with <code className="font-mono text-xs">@praman/sdk</code>; verify with <Link className="text-brand-600 underline" href="/api/v1/jwks">/api/v1/jwks</Link>.</li>
          </ol>
        </section>
      </div>
    </>

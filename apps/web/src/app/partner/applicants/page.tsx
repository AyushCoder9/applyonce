...
export default async function ApplicantsPage() {
  ...
  const rows: ApplicantRow[] = apps.map(({ a, profile, form }) => {
    ...
    if (s) { try { const p = decodeJws<PramanPayload>(decryptString(systemDek(), s.payloadEnc, `share:${s.id}`)); total = p.facts.length; verified = p.facts.filter((f) => f.source === "issuer_verified" || f.source === "provider_verified").length; self = p.facts.filter((f) => f.source === "self_declared").length; guardian = !!p.profile.guardian_acting; applicant = p.profile.display_name; } catch { /* undecodable */ } }

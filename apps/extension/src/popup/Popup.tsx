...
export function Popup() {
  ...
  const refreshAuth = useCallback(async () => {
    const res = await sendToBackground<{ ok: boolean; data?: AuthState }>({ type: "PRAMAN_AUTH_STATUS" });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (tab?.id != null) {
        setTabId(tab.id);
        try {
          const s = await sendToTab<PageStatus>(tab.id, { type: "PRAMAN_GET_STATUS" });
          setStatus(s);
        } catch { /* no content script on this page (e.g. chrome://) */ }
      }
      setLoading(false);
    })();
    const onMsg = (msg: { type: string; state?: AuthState }) => { if (msg.type === "PRAMAN_AUTH_CHANGED" && msg.state) setAuth(msg.state); };
  ...
  async function loadDocuments() {
    ...
    const res = await sendToBackground<{ ok: boolean; data?: Doc[] }>({ type: "PRAMAN_DOCUMENTS", profileId: auth.activeProfileId });
    setDocs(res.data ?? []);
  }

  async function openDocument(id: string) {
    const res = await sendToBackground<{ ok: boolean; data?: { url: string } }>({ type: "PRAMAN_DOCUMENT_URL", documentId: id });
  ...
  async function connectWithPastedCode() {
    ...
    try {
      ...
      if (!decoded.token || !decoded.expiresAt || !decoded.apiBase || !decoded.user || !decoded.profiles) throw new Error("That doesn't look like a Praman connection code.");
      const res = await sendToBackground<{ ok: boolean; data?: AuthState }>({
        type: "PRAMAN_SET_TOKEN", token: decoded.token, expiresAt: decoded.expiresAt, apiBase: decoded.apiBase, user: decoded.user, profiles: decoded.profiles,
      });
  ...
  async function disconnect() {
    await sendToBackground({ type: "PRAMAN_DISCONNECT" });
    setAuth({ connected: false });
    setDocs(null);
  }

  async function switchProfile(profileId: string) {
    await sendToBackground({ type: "PRAMAN_SET_ACTIVE_PROFILE", profileId });
  ...
  async function runFill(stepUp?: string) {
    ...
    const plan = await sendToBackground<{ ok: boolean; data?: { values: Record<string, FillValue>; labels: Record<string, string>; missing: string[] }; error?: { message: string } }>({
      type: "PRAMAN_FILL_PLAN", recipe: status.recipeId, profileId: auth.activeProfileId, keys: status.keys, stepUp,
    });
    if (!plan.ok || !plan.data) { setFill({ state: "error", message: plan.error?.message ?? "Could not reach Praman" }); return; }
    ...
    const summary = await sendToTab<FillSummary>(tabId, { type: "PRAMAN_APPLY_FILL", values: plan.data.values, labels: plan.data.labels });
  ...
  if (!auth.connected) {
    return (
      <div className="p-4 space-y-3">
        <Header />
        <p className="text-ink-2">Not connected yet. Open the Praman app and connect this browser.</p>
        <button className="praman-cta w-full py-2" onClick={() => chrome.tabs.create({ url: DEFAULT_CONNECT_URL })}>
          Open praman.in/app/extension/connect
        </button>
        <div className="praman-card p-3 space-y-2">
          ...
          <button className="praman-cta w-full py-1.5" disabled={!pasteCode.trim()} onClick={connectWithPastedCode}>Connect</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="praman-card p-3 space-y-2">
        {status?.recipeId ? (
          <>
            ...
            <p className="text-ink-2">Praman can fill {status.fieldCount} field{status.fieldCount === 1 ? "" : "s"} on this page.</p>
            {fill.state === "need-otp" && (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button className="praman-cta px-3" onClick={() => runFill(otp)}>Verify</button>
                </div>
              </div>
            )}
            ...
            {fill.state !== "need-otp" && (
              <button className="praman-cta w-full py-2" disabled={status.fieldCount === 0 || fill.state === "loading"} onClick={() => runFill()}>
      ...
      <div className="praman-card p-3 space-y-2">
        <div className="flex items-center justify-between">
          <span className="font-medium">Attach from Praman</span>
...
function Header() {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-bold">Praman Autofill</span>
    </div>
  );
}

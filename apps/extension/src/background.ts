 ...
 * place that ever calls the ApplyOnce API. Content scripts and the popup ask for data
...
async function broadcastAuthChanged(state: AuthState) {
  try { await chrome.runtime.sendMessage({ type: "APPLYONCE_AUTH_CHANGED", state }); } catch { /* no listeners open, fine */ }
}

/** Every authenticated call to the ApplyOnce API goes through here. */
async function apiFetch<T>(path: string, init: RequestInit = {}): Promise<ApiEnvelope<T>> {
  const auth = await getStoredAuth();
  if (!auth) return { ok: false, error: { code: "NOT_CONNECTED", message: "Connect the extension from the ApplyOnce app first." } };
  if (new Date(auth.expiresAt).getTime() < Date.now()) {
    await setStoredAuth(null);
    await broadcastAuthChanged({ connected: false });
    return { ok: false, error: { code: "EXTENSION_TOKEN_EXPIRED", message: "Your connection expired. Reconnect from the ApplyOnce app." } };
  }
  const res = await fetch(`${auth.apiBase}${path}`, {
    ...init,
    headers: { ...(init.headers ?? {}), authorization: `Bearer ${auth.token}` },
  });
  const json = (await res.json().catch(() => ({ ok: false, error: { code: "BAD_RESPONSE", message: "Unexpected response from ApplyOnce" } }))) as ApiEnvelope<T>;
  return json;
}

async function handle(msg: BackgroundRequest): Promise<unknown> {
  switch (msg.type) {
    case "APPLYONCE_SET_TOKEN": {
